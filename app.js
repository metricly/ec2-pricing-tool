...
function initPriceList() {
  ...
  request = https.get(
    "https://s3-us-west-2.amazonaws.com/com-netuitive-app-usw2-www/tools/pricing-tool/listprices.psv",
    response => {
...
currentTableData.on("finish", function() {
  ...
  fs.readFile(__dirname + "/currentListTable.psv", "utf8", function(
    err,
    contents
  ) {
...
app.use(
  "/bootstrap-slider",
  express.static(__dirname + "/node_modules/bootstrap-slider/dist/")
);
app.use("/jquery", express.static(__dirname + "/node_modules/jquery/dist/"));
app.use(
  "/datatables",
  express.static(__dirname + "/node_modules/datatables/media/")
);
...
app.get("/memrange", (req, res) => {
  let filteredData = getFilteredData(req);
  let range = filteredData.reduce(
    (prev, curr) => {
      // if( parseFloat(curr.memory_gb) === 0) {
      // 	console.log("0");
      // }
      prev[0] =
        parseFloat(curr.memory_gb) < prev[0]
          ? parseFloat(curr.memory_gb)
          : prev[0];
      prev[1] =
        parseFloat(curr.memory_gb) > prev[1]
          ? parseFloat(curr.memory_gb)
          : prev[1];
      return prev;
    },
...
app.get("/range", (req, res) => {
  ...
  let range = filteredData.reduce(
    (prev, curr) => {
      prev[0] =
        parseFloat(curr[field]) < prev[0] ? parseFloat(curr[field]) : prev[0];
      prev[1] =
        parseFloat(curr[field]) > prev[1] ? parseFloat(curr[field]) : prev[1];
      return prev;
    },
    [99999, 0]
  );
  ...
  if (orders && orders.length > 0) {
    ...
    if (type && type === "num") {
      filteredData = filteredData.sort(
        (a, b) => direction * (parseFloat(a[field]) - parseFloat(b[field]))
      );
    } else {
      filteredData = filteredData.sort(
        (a, b) =>
          new String(a[field]).localeCompare(new String(b[field])) * direction
      );
    }
  }
  // Paginate
...
var getFilteredData = function(req) {
  ...
  filters.forEach(
    filter =>
      (filteredData = filteredData.filter(
        row => row[filter.name] === filter.value
      ))
  );
  ...
  rangeFilters.forEach(
    filter =>
      (filteredData = filteredData.filter(
        row => row[filter.name] >= filter.min && row[filter.name] <= filter.max
      ))
  );

  checkFilters.forEach(
    filter =>
      (filteredData = filteredData.filter(
        row => row[filter.name] === filter.value
      ))
...
//   res.redirect("https://tools.metricly.com/");
// });